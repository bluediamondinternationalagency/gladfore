// Comprehensive Nigeria States and Local Government Areas (LGAs)
export interface NigeriaLocation {
  state: string;
  lgas: string[];
}

export const NIGERIA_STATES_AND_LGAS: NigeriaLocation[] = [
  {
    state: "Abia",
...
export const STATES = NIGERIA_STATES_AND_LGAS.map(location => location.state).sort();

export const getLGAsByState = (state: string): string[] => {
  const location = NIGERIA_STATES_AND_LGAS.find(loc => loc.state === state);
  return location ? location.lgas.sort() : [];
};
